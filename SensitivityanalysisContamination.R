...
log10decrement<-100000
#******************ALTER SENSITIVITY PARAMETERS***************

#E. COLI CONCENTRATIONS IN THE DIFFERENT ENVIRONMENTAL MEDIA
#Define e. coli concentration distributions
hpdistr<-fitdistr((hp$CFU_100mL/100)/log10decrement, "lognormal") # ln N(logmean, logsd) CFU/mL 
shpdistr<-fitdistr((shp$CFU_100mL/100)/log10decrement, "lognormal") # ln N(logmean, logsd) CFU/mL 
bwdistr<-fitdistr((bw$CFU_100mL/100)/log10decrement, "lognormal") # ln N(logmean, logsd) CFU/mL 
sbwdistr<-fitdistr((sbw$CFU_100mL/100)/log10decrement, "lognormal") # ln N(logmean, logsd) CFU/mL 
wdistr<-fitdistr((w$CFU_100mL/100)/log10decrement, "lognormal") # ln N(logmean, logsd) CFU/mL 
swdistr<-fitdistr((sw$CFU_100mL/100)/log10decrement, "lognormal") # ln N(logmean, logsd) CFU/mL 
naladistr<-fitdistr((nala$CFU_100mL/100)/log10decrement, "lognormal")# ln N(logmean, logsd) CFU/mL 
...
#CALCULATE ANNUAL INFECTION RISKS FOR EACH PATHWAY, PER AGE GROUP

#(bw)
n<-365
bw2yearrisk<-1-(1-bwrisk1223)^n
bw2yearinfrisk<-mc(bw2yearrisk)

#(sbw)
n<-365
sbw2yearrisk<-1-(1-sbwrisk1223)^n
sbw2yearinfrisk<-mc(sbw2yearrisk)

#(hp)
n<-365
hp2yearrisk<-1-(1-hprisk1223)^n
hp2yearinfrisk<-mc(hp2yearrisk)

#(shp)
n<-365
shp2yearrisk<-1-(1-shprisk1223)^n
shp2yearinfrisk<-mc(shp2yearrisk)

#Bathing water
n<-365
nala2yearrisk<-1-(1-nalarisk1223)^n
nala2yearinfrisk<-mc(nala2yearrisk)

#well
n<-365
w2yearrisk<-1-(1-wrisk1223)^n
w2yearinfrisk<-mc(w2yearrisk)

#swell
n<-365
sw2yearrisk<-1-(1-swrisk1223)^n
sw2yearinfrisk<-mc(sw2yearrisk)
...
#OUTPUT RESULTS OF ANNUAL INFECTION RISKS
quantile(bw2yearinfrisk, c(0.025,0.1,0.25,0.5,0.75, 0.9, 0.975))
quantile(sbw2yearinfrisk, c(0.025,0.1,0.25,0.5,0.75, 0.9, 0.975))
quantile(hp2yearinfrisk, c(0.025,0.1,0.25,0.5,0.75, 0.9, 0.975))
quantile(shp2yearinfrisk, c(0.025,0.1,0.25,0.5,0.75, 0.9, 0.975))
quantile(w2yearinfrisk, c(0.025,0.1,0.25,0.5,0.75, 0.9, 0.975))
quantile(sw2yearinfrisk, c(0.025,0.1,0.25,0.5,0.75, 0.9, 0.975))
